      ...
      <div className="navbar bg-dark-hard flex justify-between p-2">
        ...
        {/* <div className="z-50 relative items-center hidden md:block text-dark-light">
          <div ref={dropdownRef} className="flex">
            ...
            {user ? (
              <>
                <button onClick={(e) => handleDropDownFocus(open)} title={user} id="name" className="mr-20">
                  {user}
                </button>

                {open && (
                  <ul ref={ref} className="absolute z-50 bg-dark-hard p-2 rounded-b-md px-4">
                    {navlinks.map((link, index) => (
                      <a key={index} className="flex text-dark-light hover:bg-dark-soft" href={link.link}>
                        {link.title}
                      </a>
                    ))}
                    <li>
